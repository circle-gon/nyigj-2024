...
export default createLayer("games", function (this: BaseLayer) {
    ...
    function createSubpart<T extends object>(options: {
        name: string,
        action: string,
        color: string,
        ...
        const bar = createBar(() => ({
            ...
            display: () => doingAction.value === name ? `${format(gainComputed.value)}/s` : action,
            textStyle: {
                color
            },
            baseStyle: {
    ...
    const featurePart = createSubpart({
        name: "Features",
        action: "Create",
        data: (features) => [`${formatFloor(features)} features`],
        color: "pink",
        gain: () => 1,
        spend: computed(() => 1),
    ...
    const programmingPart = createSubpart({
        name: "Programming",
        action: "Program",
        data: (lines) => [`${formatFloor(lines)} lines of code`],
        color: "green",
        gain: () => 1,
        spend: computed(() => 1),
    ...
    const mechanicPart = createSubpart({
        name: "Mechanics",
        action: "Modify",
        data: (mechanics) => [`${formatFloor(mechanics)} mechanics`],
        color: "orange",
        gain: () => 1,
        spend: computed(() => 1),
